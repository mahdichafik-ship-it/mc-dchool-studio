...
**How to apply:** Create or resume the same batch when Upload & Finish is retried, link every uploaded JPEG/RAW to it, and only mark the local project finished after the server confirms that batch complete. Older unbatched uploads remain visible as compatibility data. Overall project completion is a separate owner-facing gate across all batches, failures, duplicate students, pair completeness, and acknowledged missing students.