...
**How to apply:** Keep watcher, active-student, pairing, and local-folder code independent from upload triggers. A finish operation must drain queued captures, report progress, preserve the unfinished state on any failure, and persist completion only after all eligible local files succeed.