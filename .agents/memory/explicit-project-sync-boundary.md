...
**How to apply:** Keep watcher, active-student, pairing, and local-folder code independent from upload success. Live Upload is opt-in per project, uses at most three concurrent file transfers, pauses safely offline, retains originals locally, and uses the same retry-stable photographer batch as finalization. A finish operation must pause background transfer, drain queued captures with the same limit, report per-file progress, preserve the unfinished state on any failure, and persist completion only after the server confirms the photographer batch complete.