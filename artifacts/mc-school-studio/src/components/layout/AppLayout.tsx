...
import { LayoutDashboard, FolderKanban, LogOut, Users, ShieldCheck, Settings, FileText, Mail, Menu } from 'lucide-react';
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-[100dvh] w-full bg-slate-50">
      {/* Sidebar */}
      <aside className="hidden w-64 bg-sidebar text-sidebar-foreground md:flex flex-col flex-shrink-0 sticky top-0 h-[100dvh]">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-4 md:hidden">
          <button
            ...
            className="inline-flex size-10 items-center justify-center rounded-md text-slate-700 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-600"
            aria-label="Open navigation menu"
            aria-expanded={isMobileMenuOpen}
          >
            <Menu className="size-5" />
          </button>
          {studioBrand?.logoObjectPath ? <img src={`/api/studio/branding/logo?rev=${encodeURIComponent(studioBrand.brandingUpdatedAt ?? "")}`} alt="" className="h-8 w-8 rounded-md object-contain" /> : <img src="/volume-capture-logo.png" alt="" className="h-8 w-8 rounded-md bg-black object-cover" />}
          <span className="min-w-0 truncate font-semibold text-slate-900">{studioBrand?.name ?? "Volume Capture"}</span>
        </header>
        {children}
      </main>

      <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
        <SheetContent side="left" className="flex w-[min(20rem,88vw)] flex-col border-sidebar-border bg-sidebar p-0 text-sidebar-foreground">
          <SheetTitle className="sr-only">Application navigation</SheetTitle>
          <SheetDescription className="sr-only">Navigate between Volume Capture sections.</SheetDescription>
          <div className="h-16 flex items-center px-6 border-b border-sidebar-border/50" style={{ borderTop: studioBrand ? `3px solid ${studioBrand.accentColor}` : undefined }}>
            {studioBrand?.logoObjectPath ? <img src={`/api/studio/branding/logo?rev=${encodeURIComponent(studioBrand.brandingUpdatedAt ?? "")}`} alt="" className="mr-3 h-9 w-9 rounded-md bg-white object-contain p-1" /> : <img src="/volume-capture-logo.png" alt="" className="mr-3 h-9 w-9 rounded-md bg-black object-cover" />}
            <span className="truncate font-semibold text-lg tracking-tight">{studioBrand?.name ?? "Volume Capture"}</span>
          </div>
