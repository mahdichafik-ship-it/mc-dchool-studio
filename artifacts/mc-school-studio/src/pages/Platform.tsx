...
type PlatformData = Omit<PlatformOverview, "studios"> & {
  healthSummary: {
    totalStudios: number;
...
export default function Platform() {
  ...
  const studiosNeedingAction = useMemo(
    () => data?.studios
      .filter((studio) => studio.health.severity !== "healthy")
      .sort((a, b) => {
        const rank = { critical: 0, attention: 1, healthy: 2 };
        return rank[a.health.severity] - rank[b.health.severity];
      }) ?? [],
    [data],
  );
        ...
        <section className="space-y-4">
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">A quick operations view of studios that need support or follow-up.</p>
            </div>
            <span className="text-xs text-slate-400">{data.healthSummary.totalStudios} total studios</span>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
              <CheckCircle2 className="h-5 w-5 text-emerald-700" />
              <p className="mt-3 text-2xl font-bold text-emerald-950">{data.healthSummary.healthyStudios}</p>
              <p className="text-sm text-emerald-800">Healthy studios</p>
            </div>
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              <AlertTriangle className="h-5 w-5 text-amber-700" />
              <p className="mt-3 text-2xl font-bold text-amber-950">{data.healthSummary.attentionStudios}</p>
              <p className="text-sm text-amber-800">Need attention</p>
            </div>
            <div className="rounded-xl border border-red-200 bg-red-50 p-4">
              <AlertTriangle className="h-5 w-5 text-red-700" />
              <p className="mt-3 text-2xl font-bold text-red-950">{data.healthSummary.criticalStudios}</p>
              <p className="text-sm text-red-800">Critical issues</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-4">
              <Building2 className="h-5 w-5 text-slate-600" />
              <p className="mt-3 text-2xl font-bold text-slate-900">{data.healthSummary.archivedStudios}</p>
              <p className="text-sm text-slate-500">Archived studios</p>
            </div>
          </div>
          {studiosNeedingAction.length > 0 ? (
            <div className="overflow-hidden rounded-xl border border-amber-200 bg-white shadow-sm">
              <div className="border-b border-amber-100 bg-amber-50 px-6 py-4">
                <h3 className="font-semibold text-amber-950">Studios needing action</h3>
                <p className="mt-1 text-sm text-amber-800">Open a studio to investigate the underlying condition.</p>
              </div>
              <div className="divide-y">
                {studiosNeedingAction.map((studio) => (
                  <div key={studio.id} className="flex flex-col gap-3 px-6 py-4 md:flex-row md:items-center md:justify-between">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <span className={`h-2.5 w-2.5 rounded-full ${studio.health.severity === "critical" ? "bg-red-500" : "bg-amber-500"}`} />
                        <p className="font-semibold text-slate-900">{studio.name}</p>
                        <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">{studio.health.severity}</span>
                      </div>
                      <div className="mt-2 flex flex-wrap gap-2">
                        {studio.health.alerts.map((alert) => (
                          <span key={alert.code} className={`rounded-full px-2.5 py-1 text-xs ${alert.severity === "critical" ? "bg-red-50 text-red-700" : alert.severity === "attention" ? "bg-amber-50 text-amber-800" : "bg-slate-100 text-slate-600"}`}>
                            {alert.label}
                          </span>
                        ))}
                      </div>
                    </div>
                    <Link href={`/platform/studios/${studio.id}`} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-md border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
            ...
            {data.studios.map((studio) => <article key={studio.id} className="rounded-xl border border-slate-200 p-5">
              <div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><div className="rounded-lg bg-slate-100 p-2 text-slate-600"><Building2 className="h-5 w-5" /></div><div><h3 className="font-semibold text-slate-900">{studio.name}</h3><p className="text-sm text-slate-500">{studio.owner?.displayName || studio.owner?.email || "Owner not completed"}</p></div></div><button type="button" onClick={() => openStudioEditor(studio)} className="inline-flex items-center gap-1.5 rounded-md border border-slate-200 px-2.5 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-50" aria-label={`Edit details for ${studio.name}`}><Pencil className="h-3.5 w-3.5" />Edit</button></div>
               <div className="mt-3 flex flex-wrap gap-2">
                 <span className={`rounded-full px-2.5 py-1 text-xs font-semibold capitalize ${studio.health.severity === "critical" ? "bg-red-50 text-red-700" : studio.health.severity === "attention" ? "bg-amber-50 text-amber-800" : "bg-emerald-50 text-emerald-700"}`}>{studio.health.severity}</span>
                 {studio.health.alerts.slice(0, 2).map((alert) => <span key={alert.code} className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{alert.label}</span>)}
               </div>
